...
export default function Onboarding({ onAuthorize, onSkip }: OnboardingProps) {
  return (
    <div className="flex h-dvh flex-col bg-sand-50 px-6 py-10 dark:bg-night-900">
      <div className="flex flex-1 flex-col items-center justify-center gap-6 text-center">
        ...
        <p className="text-ink-500 dark:text-mist-200">
        ...
        <div className="mt-2 flex w-full flex-col gap-4 text-left">
          {ARGUMENTS.map(({ bg, icon: Icon, title, text }) => (
            <div key={title} className="flex items-center gap-3">
              ...
              >
                <Icon size={17} className="text-ink-700 dark:text-mist-200" />
              </div>
              <div>
                <p className="text-sm font-semibold text-ink-900 dark:text-mist-50">{title}</p>
                <p className="text-[13px] text-ink-500 dark:text-mist-200">{text}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <button
          type="button"
          onClick={onSkip}
          className="h-[52px] text-sm font-semibold text-ink-500 dark:text-mist-200"
        >
