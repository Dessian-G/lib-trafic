...
export default function RoutePage({
  ...
  const { routes, loading, error } = useRoute(profile, departPos, arriveePos, profile === 'driving-car')
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="relative mt-4 h-[200px] shrink-0">
        ...
        />
        {axeBloque && (
          <div className="absolute inset-x-3 top-3 z-10 rounded-field bg-[#FBE4E2] px-3 py-2 text-xs text-[#7A1F17]">
      ...
      <div className="flex-1 overflow-y-auto px-[22px] py-4">
        {!arriveePos && <p className="text-center text-sm text-ink-500">Choisissez une destination.</p>}
        ...
        {arriveePos && departPos && loading && (
          <p className="text-center text-sm text-ink-500">Calcul de l'itinéraire…</p>
        )}
        {arriveePos && departPos && error && (
          <p className="text-center text-sm text-[#7A1F17]">{error}</p>
        )}

        {recommande && (
          <div className="flex gap-3">
            <div className="flex-1 rounded-card border-2 border-brand-600 bg-brand-50 p-3">
              <p className="text-xs font-bold text-brand-600">RECOMMANDÉ</p>
            ...
            {alternative && (
              <div className="flex-1 rounded-card bg-sand-100 p-3">
                ...
                <p className="text-xs text-ink-500">{(alternative.distanceMetres / 1000).toFixed(1)} km</p>
              </div>
            )}
          </div>
        )}
        ...
        {mode === 'bus' && recommande && (
          <div className="mt-3 rounded-field bg-sand-100 p-3 text-xs text-ink-600">
            <p className="font-semibold text-ink-700">Lignes locales</p>
